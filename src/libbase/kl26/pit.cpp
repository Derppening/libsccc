...
inline bool IsInterruptRequested_(const int channel) {
  return GET_BIT(PIT->CHANNEL[channel].TFLG, PIT_TFLG_TIF_SHIFT);
}
...
void Pit::SetIsr(const OnPitTriggerListener& isr) {
  m_isr = isr;
  const int other_channel = !m_channel;
  if (m_isr) {
    SET_BIT(PIT->CHANNEL[m_channel].TCTRL, PIT_TCTRL_TIE_SHIFT);
    if (!g_instances[other_channel] || !g_instances[other_channel]->m_isr) {
      // ISR not inited in other ch, we set it
      ::SetIsr(PIT_IRQn, IrqHandler);
      EnableIrq(PIT_IRQn);
    }
  } else {
    CLEAR_BIT(PIT->CHANNEL[m_channel].TCTRL, PIT_TCTRL_TIE_SHIFT);
    if (!g_instances[other_channel] || !g_instances[other_channel]->m_isr) {
      // ISR not inited in other ch, disable it
      DisableIrq(PIT_IRQn);
      ::SetIsr(PIT_IRQn, nullptr);
    }
  }
}
...
bool Pit::IsInterruptRequested() const {
  ...
  return IsInterruptRequested_(m_channel);
}

__ISR void Pit::IrqHandler() {
  const int channel = IsInterruptRequested_(0) ? 0 : 1;
