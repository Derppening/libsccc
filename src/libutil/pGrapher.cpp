  ...
  if (m_pd_instance->rx_buffer.size() >= m_pd_instance->rx_threshold) {
    if (m_pd_instance->rx_buffer[1]) {
      switch (m_pd_instance->rx_buffer[0]) {
        case 's': m_pd_instance->isStarted = true;
          break;
        case 'e': m_pd_instance->isStarted = false;
          break;
        case 'w': m_pd_instance->sendWatchedVarInfo();
          break;
        case 'h': m_pd_instance->sendSharedVarInfo();
          break;
        case 'c': m_pd_instance->changeSharedVars(m_pd_instance->rx_buffer);
          break;
        default:
          // not handled
          break;
...
void pGrapher::sendWatchData(void) {
  if (isStarted) {
    for (Byte i = 0; i < watchedObjMng.size(); i++) {
      ...
      m_uart.SendBuffer(static_cast<Byte *>(static_cast<ObjMng>(watchedObjMng.at(i)).obj),
                        static_cast<ObjMng>(watchedObjMng.at(i)).len);
    }
  }
}
...
void pGrapher::sendWatchedVarInfo(void) {
  ...
  m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>(",")), 1);
  // Byte n = watchedObjMng.size();
  Byte n = static_cast<Byte>(watchedObjMng.size());
  // m_uart.SendBuffer((Byte *)&n, 1);
  m_uart.SendBuffer(static_cast<Byte *>(&n), 1);
  for (Byte i = 0; i < watchedObjMng.size(); i++) {
    ObjMng temp = watchedObjMng.at(i);
    // m_uart.SendBuffer((Byte *)temp.typeName.data(), temp.typeName.size() + 1);
    m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>(temp.typeName.data())),
                      temp.typeName.size() + 1);
    // m_uart.SendBuffer((Byte *)temp.varName.data(), temp.varName.size() + 1);
    m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>(temp.varName.data())),
                      temp.varName.size() + 1);
    // m_uart.SendBuffer((Byte *)".", 1);
    m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>(",")), 1);
  }
  // m_uart.SendBuffer((Byte *)"end", 3);
  m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>("end")), 3);
}
...
void pGrapher::sendSharedVarInfo(void) {
  ...
  Byte n = static_cast<Byte>(sharedObjMng.size());
  // m_uart.SendBuffer((Byte *)&n, 1);
  m_uart.SendBuffer(static_cast<Byte *>(&n), 1);
  for (Byte i = 0; i < sharedObjMng.size(); i++) {
    ...
    m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>(temp.obj)), temp.len);
    // m_uart.SendBuffer((Byte *)".", 1);
    m_uart.SendBuffer(const_cast<Byte *>(reinterpret_cast<const Byte *>(",")), 1);
  }
  // m_uart.SendBuffer((Byte *)"end", 3);
...
void pGrapher::changeSharedVars(const std::vector<Byte> &msg) {
  ...
  int objIndex = inRangeWithAssert(0, msg[2], static_cast<uint8_t>(sharedObjMng.size()));
  for (int i = 0; i < sharedObjMng[objIndex].len; i++) {
    // ((Byte *) (sharedObjMng[objIndex].obj))[i] = msg[3 + i];
    (const_cast<Byte *>(reinterpret_cast<const Byte *>(sharedObjMng[objIndex].obj)))[i] = msg[3 + i];
  }
