...
class MiniLcd : private MiniTypeWriter {
 ...
 public:
  ...
  struct Config {
    ...
    Config(uint16_t _bgColor,
           uint16_t _txtColor,
           uint8_t _fps,
           uint8_t _floatPrecision = 3,
           bool _isTextWrap = false,
           bool _isClearLine = false)
        : bgColor(_bgColor),
          txtColor(_txtColor),
          fps(_fps),
          floatPrecision(_floatPrecision),
          isTextWrap(_isTextWrap),
          isClearLine(_isClearLine) {};
  };
